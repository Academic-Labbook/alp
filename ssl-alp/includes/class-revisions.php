...
class SSL_ALP_Revisions extends SSL_ALP_Module {
	 ...
	 * @param bool $ignore_first Ignore autogenerated revision created at the
	 *             same time as the post.
	 */
	public function get_post_edit_count( $post, $ignore_first = true ) {
		$post = get_post( $post );
		...
		if ( $edit_count > 0 && $ignore_first ) {
			/**
			 * Check if first revision was made when post was published.
			 */

			$first_revision = array_pop( $revisions );

			if ( $this->revision_was_autogenerated_on_publication( $first_revision ) ) {
				// first revision is an autogenerated duplicate, not a conscious edit
				$edit_count--;
			}
		}
	...
	public function revision_was_autogenerated_on_publication( $revision ) {
		...
		if ( is_null( $revision ) || is_null( $parent ) ) {
			// invalid
			return;
		}
