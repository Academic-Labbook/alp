...
class MultisiteCoauthorsTest extends WP_UnitTestCase {
	...
	/**
	 * Network users added to a blog should have a coauthor term created.
     *
     * @group ms-required
	 */
	function test_user_term_is_added_to_blog() {
        global $ssl_alp;

		// setup
		$author = $this->factory->user->create_and_get( array( 'user_login' => 'msauthor' ) );
        $blog_id = $this->factory->blog->create();

        // user should not yet have a term on the blog
        switch_to_blog( $blog_id );
        $term = $ssl_alp->coauthors->get_coauthor_term( $author );
        $this->assertFalse( $term );
        restore_current_blog();

        // add user to blog
        add_user_to_blog( $blog_id, $author->ID, 'subscriber' );

        // user should now have a term on the blog
        switch_to_blog( $blog_id );
        $term = $ssl_alp->coauthors->get_coauthor_term( $author );
